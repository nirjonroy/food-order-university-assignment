...
// consistent “fake price” (same id => same price)
...
function renderIndexPage() {
  ...
  catButtonsWrap.innerHTML = `
    <button class="btn btn-danger category-btn" data-target="#menu">All</button>
    ${topCategories
      .map((c) => {
  ...
  sectionsWrap.innerHTML = topCategories
    .map((c) => {
        ...
        .map(
          ...
          <div class="col-12 col-sm-6 col-lg-3 food-item" data-name="${escapeHtml(p.name)}">
            <div class="card h-100">
              <a href="product.html?id=${encodeURIComponent(p.id)}" class="text-decoration-none text-dark">
      ...
      return `
        <h2 class="mb-3" id="${secId}">${escapeHtml(c)}</h2>
  ...
  if (!id || !PRODUCTS_MAP[id]) {
    ...
    wrap.innerHTML = `
      <div class="col-12">
        ...
        <a class="btn btn-danger" href="index.html">Back to Home</a>
      </div>
    `;
    return;
  }
  ...
  wrap.innerHTML = `
    ...
    <div class="col-12 col-md-6">
      ...
      <div class="d-flex gap-2 mt-4">
        ...
        <a class="btn btn-danger buy-now" href="order.html" data-id="${p.id}">Buy Now</a>
      </div>
    </div>
  `;
}
...
function setupCategoryScrollActiveAndURL() {
  ...
  if (cat) {
    const b = Array.from(btns).find((x) => x.dataset.target === cat);
    if (b) setActiveCategoryButton(b);
  }
